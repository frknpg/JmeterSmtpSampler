package tr.com.netas.visiumload;/*
...
import tr.com.netas.visiumload.gui.SecuritySettingsPanel;
import tr.com.netas.visiumload.protocol.SendMailCommand;
 ...
 */
public class VLSmtpSampler extends AbstractSampler {
    ...
    private static final Logger log = LoggerFactory.getLogger(VLSmtpSampler.class);
     ...
     * @see org.apache.jmeter.samplers.Sampler#sample(Entry)
     */
    @Override
    ...
    private SendMailCommand createSendMailCommandFromProperties() throws AddressException {
        ...
        sendMailCmd.setSmtpServer(getPropertyAsString(VLSmtpSampler.SERVER));
        sendMailCmd.setSmtpPort(getPropertyAsString(VLSmtpSampler.SERVER_PORT));
        sendMailCmd.setConnectionTimeOut(getPropertyAsString(VLSmtpSampler.SERVER_CONNECTION_TIMEOUT));
        sendMailCmd.setTimeOut(getPropertyAsString(VLSmtpSampler.SERVER_TIMEOUT));
        ...
        sendMailCmd.setReceiverTo(getPropAsAddresses(VLSmtpSampler.RECEIVER_TO));
        sendMailCmd.setReceiverCC(getPropAsAddresses(VLSmtpSampler.RECEIVER_CC));
        sendMailCmd.setReceiverBCC(getPropAsAddresses(VLSmtpSampler.RECEIVER_BCC));
        sendMailCmd.setReplyTo(getPropAsAddresses(VLSmtpSampler.MAIL_REPLYTO));
        sendMailCmd.setSubject(calculateSubject());

        // needed for measuring sending time
        sendMailCmd.setSynchronousMode(true);

        sendMailCmd.setHeaderFields((CollectionProperty) getProperty(VLSmtpSampler.HEADER_FIELDS));
